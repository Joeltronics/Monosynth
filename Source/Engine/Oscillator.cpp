...
void Oscillator::ProcessFromFreq(Buffer& audioBuf, Buffer& freqPhaseBuf) {
...
void Oscillator::ProcessSynced(Buffer& audioBuf, Buffer& freqBuf, Buffer const& syncPhaseBuf) {
	DEBUG_ASSERT(m_sampleRate > 0.0);
...
void Oscillator::ProcessSub(Buffer& audioBuf, Buffer const& phaseBuf) {
	DEBUG_ASSERT(m_sampleRate > 0.0);
